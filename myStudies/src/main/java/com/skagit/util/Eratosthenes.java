...
public class Eratosthenes {
	...
	/**
	 * This is an interesting idea, but it actually slows things down. So we dumb it down in
	 * the first line.
	 */
	final private static int _PntKToBitEstimate(final int bitLow0, final int kLow0,
			final int bitHigh, final int kHigh, final int k) {
		 ...
		 * estimates, at kLow and kHigh. We have a problem if kLow or kHigh is 1 so we work
		 * around it. We do know that here, we are working with primes.
		 */
		...
		/*
		 * p is an estimate of how hot the true bit is running compared to our estimate, at k.
		 */
	...
	static {
		...
		if (primes == null) {
			...
			primes = new BitSetPlus(Integer.MIN_VALUE) {
				...
				@Override
				protected int kToBitEstimate(final int bitLow, final int kLow, final int bitHigh,
						final int kHigh, final int k) {
	...
	public static void testInegrator(final String[] args) {
		...
		final double d = Integrator.simpson(f, 2d, Integer.MAX_VALUE,
				/* relativeErrorThreshold= */1.0e-3);
