...
public class Integrator {
	...
	public static double lowerRiemann(final IntegrableFunction f, final double a,
			final double b, final int N) {
		double x = a;
	...
	public static double upperRiemann(final IntegrableFunction f, final double a,
			final double b, final int N) {
		double x = a;
		final double h = (b - a) / N;
		double sum = 0, fx1, fx2, fx;
	...
	public static double trapez(final IntegrableFunction f, final double a, final double b,
			final int N) {
		double x = a;
		final double h = (b - a) / N;
	...
	public static double midpoint(final IntegrableFunction f, final double a,
			final double b, final int N) {
		final double h = (b - a) / N;
	...
	public static double[] trapezPowerOfTwo(final IntegrableFunction f, final double a,
			final double b, final int N) {
	...
	public static double[][] romberg(final IntegrableFunction f, final Object util,
			final double a, final double b, final int N) {
		final double[][] values = new double[N][];
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < N; j++) {
				if (j < i) {
					...
					values[i][j] = values[i - 1][j]
							+ (values[i - 1][j] - values[i - 1][j - 1]) / (p - 1);
				}
			}
			p *= 4;
		}
	...
	public static double simpsonApproximation(final IntegrableFunction f, final double a,
			final double b, final int nIntervals0) {
		...
		for (int k = 0; k <= nIntervals; ++k) {
			/**
			 * We assume the endpoints, as given, are not singular. To re-compute the endpoints
			 * via k * h, is asking for trouble.
			 */
	...
	public static double simpson(final IntegrableFunction f, double a, double b,
	...
	/**
	 * A second degree polynomial that passes through the points (x0,y0), (x0+h,y1), and
	 * (x0+2h,y2):
	 */
	private static IntegrableFunction Par(final double x0, final double h, final double y0,
			final double y1, final double y2) {
		return new IntegrableFunction() {
			@Override
			public double valueAt(final double x) {
				return y0 + (x - x0)
						* ((y1 - y0) / h + (x - x0 - h) * (y0 - 2 * y1 + y2) / (2 * h * h));
			}
		};
	}

	/**
	 * A second degree polynomial that passes through the points (x0,f(x0)), (x0+h,f(x0+h))
	 * and (x0+2h,f(x0+2h)):
	 */
	@SuppressWarnings("unused")
	private static IntegrableFunction Pa(final double x0, final double h,
			final IntegrableFunction f) {
		return new IntegrableFunction() {
			@Override
			public double valueAt(final double x) {
				return Par(x0, h, f.valueAt(x0), f.valueAt(x0 + h), f.valueAt(x0 + 2d * h))
						.valueAt(x);
			}
		};
	}
