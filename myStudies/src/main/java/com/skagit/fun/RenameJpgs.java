...
public class RenameJpgs {
    ...
    public static void renameNegativeFiles(final MyLogger logger, final File dir, final String newParentDirPath) {
	...
	for (int k0 = 0; k0 < nBlocks; ++k0) {
		...
		final File oldFile = jpgFiles[beginningOfBlock + blockLength - 1 - k1];
		final String newName0 = String.format("%s.%03d.%s", dirName, counter++, _PictureFileExtension);
		final String newName = StaticUtilities.legalizeWindowsFilename(newName0);
		System.out.printf("\n%s%c%s", oldFile.getName(), Constants._RightArrow, newName);
	...
	if (newParentDirPath != null) {
		...
		if (dir.renameTo(newDir)) {
		    System.out.printf("\nDir %s moved to: %s.", dir.getPath(), newDir.getPath());
		} else {
		    System.out.printf("\nDir %s failed to move to: %s.", dir.getPath(), newDir.getPath());
		}
	    }
	}
    }

    private static void renameSlideFiles(final MyLogger logger, final File dir, final String newParentDirPath) {
	...
	for (int k = 0, k1 = 0, sequenceNumber = 0; k < nLines; ++k) {
	    ...
	    final String newName0 = String.format("%s.%02d.%s.%s", dirName, ++sequenceNumber, newCoreName,
		    _PictureFileExtension);
	    final String newName = StaticUtilities.legalizeWindowsFilename(newName0);
	...
	return dir.listFiles(new FileFilter() {
	    ...
	    @Override
	    public boolean accept(final File f) {
		return FilenameUtils.getExtension(f.getName()).equalsIgnoreCase(_PictureFileExtension);
	    }
	});
    }
	...
	try (Scanner scanner = new Scanner(new File(dir, dir.getName() + '.' + contentFileExtension))) {
