...
public class ClusterOfGames extends OutputLine implements Comparable<ClusterOfGames> {
	...
	if (nGames % 2 == 0) {
		...
		for (final GameAndTeamA gameAndTeamA : _gameAndTeamA) {
		    ...
		    final boolean ourTeamIsFavorite = bettingOnA == (teamA._name.equals(game.getFavoriteName()));
		    final double probGoesOurWay = ourTeamIsFavorite ? probFavoriteWins : (1d - probFavoriteWins);
	...
	_aIsHome = nA >= nB;
	final double probAWins = recurse(/* nDone= */0, /* nStillNeeded= */nPointsNeeded, /* bettingOnA= */true,
		/* specialGame= */specialGameIfBettingOnA);
	final double probBWins = recurse(/* nDone= */0, /* nStillNeeded= */nPointsNeeded, /* bettingOnA= */false,
		/* specialGame= */specialGameIfBettingOnB);
    ...
    private double recurse(final int nDone, final int nStillNeeded, final boolean bettingOnA, final Game specialGame) {
	if (nStillNeeded <= 0) {
	    return 1d;
	}
	...
	final boolean ourTeamIsFavorite = bettingOnA == teamA._name.equals(game.getFavoriteName());
	final double probGoesOurWay = ourTeamIsFavorite ? probFavoriteWins : (1d - probFavoriteWins);
	final double prob1 = probGoesOurWay
		* recurse(nDone + 1, nStillNeeded - (game == specialGame ? 2 : 1), bettingOnA, specialGame);
	final double prob2 = (1d - probGoesOurWay) * recurse(nDone + 1, nStillNeeded, bettingOnA, specialGame);
	return prob1 + prob2;
    }
	...
	for (final GameAndTeamA gameAndTeamA : _gameAndTeamA) {
	    s += String.format("\n%d.\t%s\t%s", ++k, gameAndTeamA._game.getString(), gameAndTeamA._teamA.getString());
	}
	return s;
    }
