...
public class Game extends OutputLine implements Comparable<Game> {
    ...
    public Game(final Date date, final Team awayTeam, final Team homeTeam, final int awayLine, final int homeLine,
	    final String clusterName, final String teamAName) {
	_date = date;
	_awayTeam = awayTeam;
	_homeTeam = homeTeam;
	_awayLine = awayLine;
    ...
    public static double[] linesToProbs(final int line0, final int line1, final boolean normalize) {
	...
	return String.format("%s-%s\t%s-%s\t%f", getAwayName(), awayAuxString, getHomeName(), homeAuxString,
		getProbFavoriteWins());
    }
