...
public class DivestSpreadsheet {
	...
	final static private SimpleDateFormat _MyDateFormat =
			new SimpleDateFormat("yyyy-MM-dd_hhmmsszzz");
	...
	public static void main(final String[] args) {
		...
		final File homeDir;
		if (iArg == nArgs) {
			final String homeDirPath = StringUtilities
					.getSystemProperty("user.dir", /* useSpaceProxy= */true);
		...
		final File[] datedFiles = homeDir.listFiles(new FileFilter() {
			...
			public boolean accept(final File f) {
				...
				try {
					final Date dateInName = _MyDateFormat
							.parse(fName.substring(0, fName.length() - ".txt".length()));
					return true;
		...
		final String newDateFileName =
				_MyDateFormat.format(new Date()) + ".txt";
		...
		final TreeMap<File, String> itemDirToShortName =
				ShortNameFinder.computeCaseDirToShortName(homeDir, itemDirs,
						/* abbreviate= */false);
		...
		for (int k0 = 0; k0 < nItemDirs; ++k0) {
			...
			for (int k1 = 0;; ++k1) {
				final String sheetName = CombinatoricTools.alterString(shortName,
						_MaxSheetNameLength, /* uniquifiers= */"!@#$%^&", k1);
		...
		try (PrintStream printStream = new PrintStream(dateFile)) {
			...
			for (final File dir : itemDirToShortName.keySet()) {
				...
				printStream.printf("%03d dir[%s] short[%s] sheet[%s]\n", ++k,
						dir.getName(), shortName, sheetName);
			}
