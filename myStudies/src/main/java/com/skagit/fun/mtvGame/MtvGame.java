...
public class MtvGame {
	...
	private LittleClue() {
		...
		for (int k0 = 0; k0 < nPairs; ++k0) {
			...
			if (kChosen >= nNbrs) {
			    ...
			    for (int k2 = 0; k2 < chosenNbr; ++k2, k1 = aSide.nextSetBit(k1 + 1)) {
			    }
			    a = k0;
			    b = k1;
			    if (_kPermAnswer >= 0) {
				final int aEntry = PermutationTools.kPermAndKToEntry(nPairs, _kPermAnswer, a);
				right = aEntry == b;
			    } else {
	...
	private BigClue() {
	    ...
	    for (int k = 0; k < kPerm0; ++k, kPermIt0 = _possiblePerms.nextSetBit(kPermIt0 + 1)) {
	    }
	    _kPerm = kPermIt0;
		...
		for (int k = 0; k < kPerm1; ++k, kPermIt1 = _possiblePerms.nextSetBit(kPermIt1 + 1)) {
		}
	...
	public void process() {
	    ...
	    NextPerm: for (int kPerm = _possiblePerms.nextSetBit(0); kPerm >= 0; kPerm = _possiblePerms
		    .nextSetBit(kPerm + 1)) {
		int nRight = 0, nWrong = 0;
		for (int k = 0; k < nPairs; ++k) {
		    if (PermutationTools.kPermAndKToEntry(nPairs, kPerm, k) == perm0[k]) {
			if (++nRight > _nRight) {
	...
	public String getString() {
	    ...
	    return String.format("%s: nRight[%d]", CombinatoricTools.getString(perm), _nRight);
	}
	...
	for (boolean didSomething = true; didSomething;) {
		...
		while (true) {
		    ...
		    for (int k = extnsn.nextSetBit(0); k >= 0; k = extnsn.nextSetBit(k + 1)) {
			nbrs.or(side[k]);
		    }
		    if (nbrs.cardinality() <= nInExtnsn) {
			/** Outside of extnsn, nobody can be matched to any of nbrs. */
			for (int k = extnsn.nextClearBit(0); k < nPairs; k = extnsn.nextClearBit(k + 1)) {
	...
	boolean madeChange = false;
	final int nPairs = getNPairs();
	for (int kPerm = _possiblePerms.nextSetBit(0); kPerm >= 0; kPerm = _possiblePerms.nextSetBit(kPerm + 1)) {
	    for (int a = 0; a < nPairs; ++a) {
		final int b = PermutationTools.kPermAndKToEntry(nPairs, kPerm, a);
		/** In kPerm, a is matched to b. */
		if (!_aSide[a].get(b)) {
		    /**
		     * Since b is not a possible match of a in _aSide, we eliminate kPerm.
		     */
		    _possiblePerms.clear(kPerm);
		    madeChange = true;
		    break;
	...
	s += String.format("\nCounts:\t[GRY]=[%d,%d,%d] (Check(%d)), nPermsLeft[%d]", counts[0], counts[1], counts[2],
		check, counts[3]);
	if (counts[3] == 1) {
	...
	for (int kTrial = 0; kTrial < nTrials; ++kTrial) {
		...
		if (printRounds) {
		    ...
		    System.out.printf("Round # %d.\tLittleClue %s yields\n%s", nRounds, littleClue.getString(),
			    mtvGame.getString());
		}
		if (mtvGame.isSolved()) {
		    System.out.printf("Solved! %d round%s, after LittleClue", nRounds, nRounds == 0 ? "" : "s");
	...
	for (int kRound = 0; kRound < nResults; ++kRound) {
	    ...
	    System.out.printf("\n%d Rounds: nAfterLittle[%d] nAfterBig[%d]", kRound, nAfterLittle, nAfterBig);
	}
    }
