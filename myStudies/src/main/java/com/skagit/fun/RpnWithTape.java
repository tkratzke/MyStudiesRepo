		...
		try (Scanner scanner = new Scanner(file)) {
			while (scanner.hasNextLine()) {
				...
				if (lineLc.startsWith("print")) {
					...
					/**
					 * We print this out assuming that topOfStack is really a
					 * numberOfMinutes, and we're interested in the mm:ss format. This
					 * is just an example.
					 */
					final int nSeconds = (int) Math.round(topOfStack * 60d);
					System.out.printf("%s%s %s", havePrintLine ? "\n" : "", caption,
							TimeUtilities.durationInSecondsToMinsSecsString(nSeconds,
									/* nDigitsForMinutes= */2));
					havePrintLine = true;
					if (printSum) {
						final int stackSumSeconds = (int) Math.round(stackSum * 60d);
						System.out.printf(", Cum[%s].",
								TimeUtilities.durationInSecondsToMinsSecsString(
										stackSumSeconds, /* nDigitsForMinutes= */3));
					} else {
						System.out.printf(".");
					}
					continue;
