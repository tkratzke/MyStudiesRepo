...
public class Interleave {
	...
	FilePair(final File inFile0, final File inFile1, final Random r) {
		...
		try (PDDocument inDocument1 = PDDocument.load(_inFile1)) {
			...
			try (final PDDocument outDocument = new PDDocument()) {
				...
				final PDPage pdPage0 = outDocument.importPage(inDocument0.getPage(k));
				pdPage0.setAnnotations(null);
				final PDPage pdPage1 = outDocument.importPage(inDocument1.getPage(nPages - 1 - k));
				pdPage1.setAnnotations(null);
			    }
			    final String outDocumentName = String.format("%s_%03d_%s.pdf",
				    FilenameUtils.getBaseName(_inFile0.getName()), Math.abs(r.nextLong()) % 1000,
	...
	final String getString() {
	    ...
	    final String s = String.format("In0: %s\nIn1: %s\nOut: %s", inFile0String, inFile1String, outFileString);
	    return s;
	}
	...
	if (!FilenameUtils.getExtension(outName).equalsIgnoreCase("pdf")) {
	    outName = FilenameUtils.getBaseName(outName) + FilenameUtils.EXTENSION_SEPARATOR + "pdf";
	}
	...
	for (int k = 0; k < nFilePairs; ++k) {
		...
		file0Name = FilenameUtils.getBaseName(file0Name) + FilenameUtils.EXTENSION_SEPARATOR + "pdf";
	    }
	    String file1Name = args[iArg++];
	    if (!FilenameUtils.getExtension(file1Name).equalsIgnoreCase("pdf")) {
		file1Name = FilenameUtils.getBaseName(file1Name) + FilenameUtils.EXTENSION_SEPARATOR + "pdf";
	    }
