...
public class CodeComputer {
	...
	private static class FullReduction {
		...
		private int addToReducedCodeListAndCountTrails(final Code code) {
			...
			final Code oneBack = nExistingCodes >= 1
					? _reducedCodeList.get(nExistingCodes - 1)
					: null;
			if ((code == Code.EM || code == Code.TWO)
					&& (oneBack == Code.EM || oneBack == Code.TWO)) {
				if (oneBack == Code.EM) {
			...
			/** ONE-EM-ONE -> ONE-ONE. */
			final Code twoBack = nExistingCodes >= 2
					? _reducedCodeList.get(nExistingCodes - 2)
					: null;
			...
			final Code threeBack = nExistingCodes >= 3
					? _reducedCodeList.get(nExistingCodes - 3)
					: null;
			...
			if (twoBack == Code.TWO && oneBack == Code.ONE && code == Code.TWO) {
				 ...
				 * TWO-ONE-ONE.
				 */
				if (threeBack == Code.TWO && twoBack == Code.ONE && oneBack == Code.ONE
						&& code == Code.TWO) {
					replaceWithMagicTriple = true;
				} else if (threeBack == Code.TWO && twoBack == Code.ONE && oneBack == Code.ONE
						&& code == Code.EM) {
					replaceWithMagicTriple = true;
				} else if (threeBack == Code.EM && twoBack == Code.ONE && oneBack == Code.ONE
						&& code == Code.TWO) {
					replaceWithMagicTriple = true;
				}
				if (replaceWithMagicTriple) {
					...
					predecessor = nExistingCodes >= 4
							? _reducedCodeList.get(nExistingCodes - 4)
							: null;
				}
			}
	...
	/** The ctor. */
	private CodeComputer(final ArrayList<Code> codeList,
			final BuildUseIgnore buildUseIgnore) {
		final boolean buildOrUse = buildUseIgnore == BuildUseIgnore.BUILD
				|| buildUseIgnore == BuildUseIgnore.USE;
		...
		if (buildOrUse) {
			...
			if (lookedUp != null) {
				_nTrailsAndCodeW = new NTrailsAndCodeW(nTrailsLost + lookedUp._nTrails,
						lookedUp._codeW);
				return;
		...
		_nTrailsAndCodeW = new NTrailsAndCodeW(nTrailsLost + forReduced._nTrails,
				forReduced._codeW);
	}
	...
	NTrailsAndCodeW solveForReduced(final BuildUseIgnore buildUseIgnore) {
		...
		if (nCodesInReducedList == 0) {
			...
			if (idxOfTWO == -1) {
				 ...
				 * nEarlyTrails, because we start with the end of the latest of the early trails.
				 */
				 ...
				 * together; note that if they are not the same trail, _codeW = ZERO and we reduce
				 * the number of trails. If they are the same trail, which is detected by
				...
				if (earlyTrailToW && lateTrailToW) {
					...
					final boolean lastIsEM = reducedCodeList
							.get(nCodesInReducedList - 1) == Code.EM;
	...
	private static Comparator<ArrayList<Code>> _CodeListComparator = new Comparator<>() {
		...
		public int compare(final ArrayList<Code> codeList0, final ArrayList<Code> codeList1) {
			...
			final ArrayList<Code> lesser0 = _Lexicographic(codeList0, codeList0R) <= 0
					? codeList0
					: codeList0R;
			final ArrayList<Code> codeList1R = new ArrayList<>(codeList1);
			Collections.reverse(codeList1R);
			final ArrayList<Code> lesser1 = _Lexicographic(codeList1, codeList1R) <= 0
					? codeList1
					: codeList1R;
			return _Lexicographic(lesser0, lesser1);
		}
	};
	private static int _Lexicographic(final ArrayList<Code> codeList0,
			final ArrayList<Code> codeList1) {
		final int len0 = codeList0.size();
	...
	private static NTrailsAndCodeW getFromIrreducibleCodeLists(
			final ArrayList<Code> codeList) {
	...
	final static TreeMap<ArrayList<Code>, NTrailsAndCodeW> _IrreducibleCodeLists = new TreeMap<>(
			_CodeListComparator);
	static {
		ArrayList<ArrayList<Code>> oldCodeLists = new ArrayList<>();
		final ArrayList<Code> emptyCodeList = new ArrayList<>();
		final CodeComputer codeComputer0 = new CodeComputer(emptyCodeList,
				BuildUseIgnore.IGNORE);
		...
		for (;;) {
			...
			for (int k = 0; k < nOldCodeLists; ++k) {
				...
				for (final Code newCode : EnumSet.allOf(Code.class)) {
					...
					final FullReduction fullReduction = new FullReduction(oldCodeListClone,
							/* buildOrUse= */true);
	...
	private static String iCaseToString(final int[] lengths, final Code[] componentCodes,
			final int iCase) {
	...
	public static void main(final String[] args) {
		...
		int nIrreducibles = 0;
		final TreeMap<ArrayList<Code>, NTrailsAndCodeW> atMostOneTWO = new TreeMap<>(
				_CodeListComparator);
		...
		for (int iPass = 0; iPass < 2; ++iPass) {
			...
			for (int iCase = 1; iCase < nCases; ++iCase) {
				...
				final FullReduction fullReduction = new FullReduction(codeList,
						/* buildOrUse= */false);
				final ArrayList<Code> reducedCodeList = fullReduction._reducedCodeList;
				...
				final CodeComputer codeComputer = new CodeComputer(codeList,
						BuildUseIgnore.IGNORE);
				final boolean isRegistered = atMostOneTWO.get(codeList) != null;
				if (irreducible && !isRegistered) {
					final ArrayList<Code> reversed = new ArrayList<>(codeList);
					Collections.reverse(reversed);
					final ArrayList<Code> codeListToUse = _Lexicographic(codeList, reversed) <= 0
							? codeList
							: reversed;
				...
				if (!irreducible) {
					str2 = String.format("[%s]",
							codeListToString(reducedCodeList, /* reverse= */true));
				} else {
					str2 = "";
				}
				if (iPass == 0) {
					...
					if (irreducible && !isRegistered) {
						System.out.printf("%" + nDigitsA + "d. %" + nDigitsB + "d. %s", iCase,
								++kIrreducibles, str1);
					} else {
						System.out.printf("%" + nDigitsA + "d. %" + nDigitsB + "s  %s", iCase, "",
								str1);
					}
					...
					System.out.printf(" %s %s", codeComputer.getString1(),
							codeComputer.getString2());
				}
			}
		}

		/**
		 * _IrreducibleCodeLists, miscTestCodeLists, and atMostOneTWO are computed. Print out
		 * the results of computing CodeComputers for them.
		 */
					...
					if (iPass1 == 0) {
						...
						if (forIrreducibles || forAtMostOneTWO) {
							if (k == nTestCodeLists - 1
									|| codeList.size() != codeLists.get(k + 1).size()) {
