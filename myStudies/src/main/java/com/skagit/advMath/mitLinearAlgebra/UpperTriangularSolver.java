...
class UpperTriangularSolver extends MitLinearAlgebraCore {
	...
	UpperTriangularSolver(final double threshold, final double[][] u,
			final double[][] rhsRows) {
		super(threshold);
		_u = u;
		if (_u == null) {
		...
		/**
		 * Find the pivot variables for each non-zero row. The number of these
		 * is the rank, and the number of free variables is the number of
		 * variables minus the rank.
		 */
		...
		for (int j0 = 0, i0 = 0; j0 < n; ++j0) {
			...
			/**
			 * 0-fill unless j0 is the pivot for row i0, in which case, we fill
			 * with the rhs.
			 */
		...
		for (int i0 = rank - 1, j0 = n - 1, jFree = nFree - 1; j0 >= 0; --j0) {
			/**
			 * If we've run out of rows or j0 is not the pivot index for this row,
			 * j0 is a free variable.
			 */
			if (i0 < 0 || j0 != pivotClmns[i0]) {
				/**
				 * j0 is a free variable. If !computingNullSpace, it's already set
				 * to 0 or Nan.
				 */
				if (computingNullSpace) {
					/**
					 * j0 is the jFree-th free variable, and is associated with the
					 * jFree-th basis vector of the nullspace. Since the value for
					 * this variable is 0 for the nullspace basis vectors that
					 * correspond to the other free variables, we only update the
					 * jFree-th RHS. Moreover, the value of the variable is 1 so there
					 * is no multiplication, just the value of the coefficient itself.
					 * Finally, note that we must update for i <= i0, not just i < i0.
					 */
					_xRows[j0][jFree] = 1d;
					for (int i = 0; i <= i0; ++i) {
	...
	private static void process(final double[][] a,
			final double[][] rhsRows) {
		final UpperTriangularSolver uts1 =
				new UpperTriangularSolver(a, rhsRows);
		System.out.printf("\n%s", uts1.getString());
		if (rhsRows != null) {
			final UpperTriangularSolver uts2 =
					new UpperTriangularSolver(a, /* rhsRows= */null);
			System.out.printf("\n%s", uts2.getString());
		}
	}
	...
	public static void main(final String[] args) {
		...
		final UpperTriangularSolver uts1 =
				new UpperTriangularSolver(u, rhsRows);
