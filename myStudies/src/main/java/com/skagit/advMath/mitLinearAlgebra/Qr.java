...
public class Qr extends MitLinearAlgebraCore {
	...
	public Qr(final double threshold, final double[][] a) {
		...
		for (int j0 = 0; j0 < n; ++j0) {
			...
			if (magnitude > _threshold) {
				...
				/**
				 * aj0 is not worth having its own QClmn so we do not add a new
				 * QClmn. rClmn was allocated room to hold a coefficient for such a
				 * QClmn and we need to put a 0 in it since it would refer to the
				 * next QClmn.
				 */
				rClmn[nQClmns] = 0d;
			}
	...
	public double[][] getNullSpace() {
		final UpperTriangularSolver uts =
				new UpperTriangularSolver(_threshold, _r, //
						/* rhsRows= */ null);
		return uts._xRows;
	}
	...
	public double[][] solve(final double[][] rhsRows) {
		...
		if (rhsRows == null || rhsRows.length != m || rhsRows[0] == null ||
				rhsRows[0].length == 0) {
			return new double[0][];
		}
		...
		final UpperTriangularSolver uts =
				new UpperTriangularSolver(_threshold, _r, qT_rhs);
		return uts._xRows;
	}
	...
	private static void process(final double[][] a) {
		...
		final double[][] rhsRows =
				transpose(new double[][] { rhsClmn0, rhsClmn1, rhsClmn2 });
	...
	public static void main(final String[] args) {
		final RandomMatrixBuilder rmb = new RandomMatrixBuilder();
		final double[][] a = rmb.buildMatrix(/* m= */8, /* n= */6,
