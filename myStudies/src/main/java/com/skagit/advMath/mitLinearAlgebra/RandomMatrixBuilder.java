...
class RandomMatrixBuilder extends MitLinearAlgebraCore {
    ...
    RandomMatrixBuilder(final long seed, final int lowNudge, final int highNudge, final int lowMult,
	    final int highMult) {
	_random = new Random();
     ...
     * </pre>
     */
    double[][] buildMatrix(final int mIn, final int nIn, final int[] indClmnsArrayIn, final boolean upperTriangular) {
	...
	for (int j = 0; j < n; ++j) {
	    ...
	    final int lub = lubIdx == indClmnsArray.length ? n : indClmnsArray[lubIdx];
	    for (; j < lub; ++j) {
	...
	final double[][] mtx0 = buildMatrix(m, m, /* indClmns= */getInitial(m), /* upperTriangular= */false);
    ...
    double[][] getRandomMatrixFromSvd(final int m, final int n, final double[] sigmas) {
    ...
    double[] getRandomLinearCombination(final double[][] vectors, final int m) {
    ...
    double[] getRandomIndependentVector(final double[][] vectors, final int m) {
	final int n = getNVectors(vectors);
	if (n == 0) {
	    final double[] vector = new double[m];
	    for (int i = 0; i < m; ++i) {
	...
	/**
	 * If we have m pivots, we cannot get an independent column. After that check,
	 * record the pivot rows and build an array of columns to take a linear
	 * combination of.
	 */
	...
	for (int i = pivotRows.nextClearBit(0); i < m; i = pivotRows.nextClearBit(i + 1)) {
