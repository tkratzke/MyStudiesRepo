...
public class DirsTracker {
	...
	final private static String _GamesDirName = "GameDirs";
	...
	static {
		final String userDirPath = Statics.getSystemProperty("user.dir",
				/* useSpaceProxy= */true);
		...
		final boolean clearLogDir = Statics.getSystemProperty("ClearLogDir",
				/* useSpaceProxy= */true) == null;
		if (clearLogDir) {
	...
	private static File computeRunDir() {
		...
		/** Unless overridden as a VM argument, _RunDir is set to _UserDir. */
		final String runDirPath = Statics.getSystemProperty("run.dir",
				/* useSpaceProxy= */true);
		if (runDirPath == null) {
			return userDir;
		}
